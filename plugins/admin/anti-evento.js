...
export default {
  commands: ["antievento"],
  description: "Bloqueia eventos no grupo",
  ...
  async run(client, m, args) {
    ...
    if (!val || !["on", "off"].includes(val)) {
      return m.reply("❌ Use: .antievento on | .antievento off | .antievento on --ban");
    }

    const banMode = args.includes("--ban");
    setGroup(m.jid, "antievento", val === "on");
    setGroup(m.jid, "antieventoBan", val === "on" ? banMode : false);
