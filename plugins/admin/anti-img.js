...
export default {
  commands: ["antiimg"],
  description: "Bloqueia imagens no grupo",
  ...
  async run(client, m, args) {
    ...
    if (!val || !["on", "off"].includes(val)) {
      return m.reply("❌ Use: .antiimg on | .antiimg off | .antiimg on --ban");
    }

    const banMode = args.includes("--ban");
    setGroup(m.jid, "antiimg", val === "on");
    setGroup(m.jid, "antiimgBan", val === "on" ? banMode : false);
