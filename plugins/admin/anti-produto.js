...
export default {
  commands: ["antiproduto"],
  description: "Bloqueia mensagens de produto no grupo",
  ...
  async run(client, m, args) {
    ...
    if (!val || !["on", "off"].includes(val)) {
      return m.reply("❌ Use: .antiproduto on | .antiproduto off | .antiproduto on --ban");
    }

    const banMode = args.includes("--ban");
    setGroup(m.jid, "antiproduto", val === "on");
    setGroup(m.jid, "antiprodutoBan", val === "on" ? banMode : false);
